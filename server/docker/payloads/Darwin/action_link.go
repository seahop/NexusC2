...
// Link commands for connecting to TCP child agents
...
// LinkCommand handles the 'link' command for connecting to TCP agents
...
func (c *LinkCommand) Execute(ctx *CommandContext, args []string) CommandResult {
	...
	default:
		// SMB is not supported on macOS - only TCP linking is available
		return CommandResult{
