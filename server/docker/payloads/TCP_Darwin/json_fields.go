// server/docker/payloads/TCP_Darwin/json_fields.go
//go:build darwin
// +build darwin
...
// Decrypted at runtime using xorKey from main.go

func init() {
	// Decrypt JSON field names at startup using xorKey from main.go
	// xorDecrypt is defined in encryption.go
	jfCT = xorDecrypt(jfCT, xorKey)
	jfCO = xorDecrypt(jfCO, xorKey)
	jfCI = xorDecrypt(jfCI, xorKey)
	jfCB = xorDecrypt(jfCB, xorKey)
	jfAI = xorDecrypt(jfAI, xorKey)
	jfFN = xorDecrypt(jfFN, xorKey)
	jfOF = xorDecrypt(jfOF, xorKey)
	jfRP = xorDecrypt(jfRP, xorKey)
	jfCC = xorDecrypt(jfCC, xorKey)
	jfTC = xorDecrypt(jfTC, xorKey)
	jfDA = xorDecrypt(jfDA, xorKey)
	jfAR = xorDecrypt(jfAR, xorKey)
	jfTS = xorDecrypt(jfTS, xorKey)
	jfJI = xorDecrypt(jfJI, xorKey)
	jfOU = xorDecrypt(jfOU, xorKey)
	jfEC = xorDecrypt(jfEC, xorKey)
	jfER = xorDecrypt(jfER, xorKey)
	jfV = xorDecrypt(jfV, xorKey)
	jfT = xorDecrypt(jfT, xorKey)
	jfTPL = xorDecrypt(jfTPL, xorKey)
	jfP = xorDecrypt(jfP, xorKey)
	jfBD = xorDecrypt(jfBD, xorKey)
	jfCD = xorDecrypt(jfCD, xorKey)
	jfST = xorDecrypt(jfST, xorKey)
	jfNC = xorDecrypt(jfNC, xorKey)
	jfSI = xorDecrypt(jfSI, xorKey)
	jfSG = xorDecrypt(jfSG, xorKey)
	jfSD = xorDecrypt(jfSD, xorKey)
	jfCT2 = xorDecrypt(jfCT2, xorKey)
	jfET = xorDecrypt(jfET, xorKey)
	jfMD = xorDecrypt(jfMD, xorKey)

	// Initialize fallback values for any empty fields
	initJSONFields()
}
var (
