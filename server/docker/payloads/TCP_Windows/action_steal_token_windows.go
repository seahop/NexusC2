...
func (c *StealTokenCommand) listStoredTokens() CommandResult {
	...
	for name, metadata := range globalTokenStore.Metadata {
		...
		details := ""
		if metadata.Source == stSourceStolen {
			procName := metadata.ProcessName
			if len(procName) > 15 {
				procName = procName[:12] + stDots
			}
			details = procName + stPidPrefix + strconv.Itoa(int(metadata.SourcePID)) + ")"
		} else if metadata.Source == stSourceCreated {
			details = stLogonPre2 + metadata.LogonType
		}
