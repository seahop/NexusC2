...
graph TB
    subgraph Host["Host Network"]
        AH[Agent Handler Service<br/>gRPC :50051]
    end

    subgraph Bridge["Docker Bridge: c2_internal (172.28.0.0/16)"]
        direction TB
        DB[(PostgreSQL<br/>172.28.0.2:5432<br/>internal only)]
        WS[WebSocket Service<br/>172.28.0.3:3131<br/>exposed]
        REST[REST API<br/>172.28.0.6:8443<br/>exposed]
        BUILD[Builder<br/>172.28.0.5<br/>on-demand]
    end

    subgraph External["External Connections"]
        GUI[GUI Client]
        API[API Client]
        AGENTS[Agents<br/>HTTP/HTTPS Listeners]
    end

    GUI -->|WSS :3131| WS
    API -->|HTTPS :8443| REST
    AGENTS -->|HTTP/HTTPS| AH
...
| `config.toml` | Malleable profile configuration (see below) |
| `.env` | Environment variables |
| `db/.secrets/.env` | Database credentials |
| `agent.yaml` | Agent handler config |

### Malleable Profile (config.toml)

The `config.toml` file controls traffic customization, including:

- **HTTP routes** - Custom URIs, methods, and parameters for agent communication
- **Server headers** - Masquerade as nginx, Apache, IIS, etc.
- **Payload defaults** - Sleep intervals, jitter, HTTP headers, user agent
- **Malleable commands** - Rename protocol keywords to avoid detection
- **SMB link settings** - Named pipe names and encrypted field names

See [Malleable Profiles]({{< ref "malleable-profiles.md" >}}) for complete documentation.
