...
**Key Features:**
- Named pipe connections between agents
...
│           ▼                                                     │
│  ┌──────────────────┐                                           │
│  │   Edge Agent     │ ← HTTP/HTTPS connection                   │
│  │   (Windows)      │                                           │
│  └────────┬─────────┘                                           │
│           │                                                     │
│  Internal │ Network ───────────────────────────────────────     │
...
┌─────────┐        ┌─────────┐        ┌─────────┐
│ Server  │───────▶│  Edge   │───────▶│   SMB   │
│         │  GET   │  Agent  │  Pipe  │  Agent  │
└─────────┘        └─────────┘        └─────────┘

1. Server receives GET from edge agent
2. Server includes link commands in response:
   {
     ...
     "lr": [                          // link handshake responses
       {"r": "3", "p": "<handshake response>"}
     ]
   }

3. Edge agent forwards to each linked agent
...
┌─────────┐        ┌─────────┐        ┌─────────┐
│   SMB   │───────▶│  Edge   │───────▶│ Server  │
│  Agent  │  Pipe  │  Agent  │  POST  │         │
└─────────┘        └─────────┘        └─────────┘

1. SMB agent sends encrypted results via pipe
2. Edge agent collects in outbound queue
3. Edge agent includes in POST:
   {
     "agent_id": "<edge_id>",
     "results": [<edge results>],
     "ld": [                          // link data
       {"r": "1", "p": "<encrypted results>"},
       {"r": "2", "p": "<encrypted results>"}
     ],
     "lu": ["3"]                       // unlink notifications
   }
